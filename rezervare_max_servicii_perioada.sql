...
BEGIN
    IF p_data_inceput > p_data_sfarsit OR p_data_inceput IS NULL OR p_data_sfarsit IS NULL THEN
        RAISE_APPLICATION_ERROR(-20001, 'Perioad� invalid�.');
    END IF;
...
EXCEPTION
    WHEN NO_DATA_FOUND THEN
        DBMS_OUTPUT.PUT_LINE('Nu exist� rezerv�ri cu servicii suplimentare incluse �n perioada dat�.');
    RETURN NULL;
    
    WHEN TOO_MANY_ROWS THEN
        DBMS_OUTPUT.PUT_LINE('Exist� mai multe rezerv�ri cu num�rul maxim de servicii din perioada dat�.');
    RETURN NULL;
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('  Dat� rezervare: ' || v_data_rezervare);
    DBMS_OUTPUT.PUT_LINE('  Perioad�: ' || v_data_inceput || ' - ' || v_data_sfarsit);
    DBMS_OUTPUT.PUT_LINE('  Num�r persoane: ' || v_numar_persoane);
END;
