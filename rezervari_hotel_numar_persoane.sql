...
BEGIN
    IF p_numar_persoane < 0 OR p_numar_persoane IS NULL THEN
        RAISE_APPLICATION_ERROR(-20002, 'Num�r persoane invalid.');
    END IF;
    ...
    LOOP
        ...
        DBMS_OUTPUT.PUT_LINE('Dat� rezervare: ' || x.data_rezervare);
        DBMS_OUTPUT.PUT_LINE('Perioad�: ' || x.data_inceput || ' - ' || x.data_sfarsit);
        DBMS_OUTPUT.PUT_LINE('Nume client: ' || x.nume || ' ' || x.prenume);
        DBMS_OUTPUT.PUT_LINE('Email client: ' || x.email);
        DBMS_OUTPUT.PUT_LINE('Suma total�: ' || x.suma_totala);
        DBMS_OUTPUT.PUT_LINE('');
    END LOOP;
EXCEPTION
    WHEN e_hotel_inexistent THEN
        DBMS_OUTPUT.PUT_LINE('Hotelul nu exist�.');
        
    WHEN e_fara_rezervari THEN
        DBMS_OUTPUT.PUT_LINE('Nu exist� rezerv�ri pentru hotelul dat cu num�rul de persoane date.');
...
-- apel pentru care nu exist� rezerv�ri
BEGIN
